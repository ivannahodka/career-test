...
function goDirectL2() {
  ...
  const key = sel.value;
  if (!key) return;
  // Science profession selected
  if (key.startsWith('science:')) {
    const sciKey = key.slice('science:'.length);
    const hasSciL2 = hasL2Data(sciKey);
    if (hasSciL2) {
      ...
      document.getElementById('l2-result').style.display = 'none';
      const d = getL2Data();
      if (!d) { startScienceDomain(); return; }
      l2Current = 0;
      l2Answers = Array.from({length: d.questions.length}, () => new Set());
      const sul2 = d.ui || {};
      document.getElementById('l2-to-l1').textContent = (sul2.l2back || '← Назад') + ' (' + hu('backHome') + ')';
  ...
  // Business profession selected
  if (key.startsWith('business:')) {
    const bizKey = key.slice('business:'.length);
    const hasBizL2 = hasL2Data(bizKey);
    if (hasBizL2) {
      ...
      document.getElementById('l2-result').style.display = 'none';
      const d = getL2Data();
      if (!d) { startBusinessDomain(); return; }
      l2Current = 0;
      l2Answers = Array.from({length: d.questions.length}, () => new Set());
      const bul2 = d.ui || {};
      document.getElementById('l2-to-l1').textContent = (bul2.l2back || '← Назад') + ' (' + hu('backHome') + ')';
      document.getElementById('l2-quiz').style.display = 'block';
      renderL2Question();
    } else {
      document.getElementById('home').style.display = 'none';
  ...
  // Creative profession selected — jump directly to creative L2
  if (key.startsWith('creative:')) {
    const creativeKey = key.slice('creative:'.length);
    const hasCreativeL2 = typeof CREATIVE_L2 !== 'undefined' && CREATIVE_L2[creativeKey];
    const itAlias = (typeof CREATIVE_TO_IT_L2 !== 'undefined') && CREATIVE_TO_IT_L2[creativeKey];
    if (!hasCreativeL2 && !itAlias) {
      // no L2 yet — fall back to general creative quiz
      document.getElementById('home').style.display = 'none';
      startCreativeDomain();
      return;
    }
    // If native creative L2 exists — use it; otherwise fall back to IT alias
    if (hasCreativeL2) {
      activeDomain = 'creative';
      l2ProfKey = creativeKey;
    } else {
      activeDomain = 'it';
      l2ProfKey = itAlias;
    }
    ...
    document.getElementById('l2-result').style.display = 'none';
    const d = getL2Data();
    if (!d) { startCreativeDomain(); return; }
    l2Current = 0;
    l2Answers = Array.from({length: d.questions.length}, () => new Set());
    const cu = (typeof CREATIVE_I18N !== 'undefined' && (CREATIVE_I18N[lang] || CREATIVE_I18N.en).ui) || I18N[lang].ui;
    document.getElementById('l2-to-l1').textContent = (cu.l2back || cu.l2directBack) + ' (' + hu('backHome') + ')';
    document.getElementById('l2-quiz').style.display = 'block';
    renderL2Question();
    return;
  }
  // Pedagogy profession selected
  if (key.startsWith('pedagogy:')) {
    const pedKey = key.slice('pedagogy:'.length);
    const hasPedL2 = hasL2Data(pedKey);
    if (hasPedL2) {
      ...
      document.getElementById('l2-result').style.display = 'none';
      const d = getL2Data();
      if (!d) { startPedagogyDomain(); return; }
      l2Current = 0;
      l2Answers = Array.from({length: d.questions.length}, () => new Set());
      const pul2 = d.ui || {};
      document.getElementById('l2-to-l1').textContent = (pul2.l2back || '← Назад') + ' (' + hu('backHome') + ')';
      document.getElementById('l2-quiz').style.display = 'block';
      renderL2Question();
    } else {
      document.getElementById('home').style.display = 'none';
  ...
  if (key.startsWith('engineering:')) {
    const engKey = key.slice('engineering:'.length);
    const hasEngL2 = hasL2Data(engKey);
    if (hasEngL2) {
      ...
      document.getElementById('l2-result').style.display = 'none';
      const d = getL2Data();
      if (!d) { startEngineeringDomain(); return; }
      l2Current = 0;
      l2Answers = Array.from({length: d.questions.length}, () => new Set());
      const eul2 = d.ui || {};
      document.getElementById('l2-to-l1').textContent = (eul2.l2back || '← Назад') + ' (' + hu('backHome') + ')';
      document.getElementById('l2-quiz').style.display = 'block';
      renderL2Question();
    } else {
      document.getElementById('home').style.display = 'none';
      startEngineeringDomain();
    }
    return;
  }

  if (!hasL2Data(key)) return;
  // Show quiz container first (needed for DOM elements)
  document.getElementById('home').style.display = 'none';
  document.getElementById('quiz-container').style.display = '';
  document.getElementById('result').style.display = 'none';
  document.getElementById('quiz').style.display = 'none';
  ...
  l2Current = 0;
  l2Answers = Array.from({length: d.questions.length}, () => new Set());
  document.getElementById('l2-to-l1').textContent = I18N[lang].ui.l2directBack;
  document.getElementById('l2-quiz').style.display = 'block';
  renderL2Question();
}
